...
Page/tile scroll coordinates wrap to the configured tilemap dimensions. Sprite
X/Y fields are signed ten-bit coordinates and clip at the LCD edges.
...
|---:|---|---|
| `0x702a` | `P_BLENDING` | Global 25/50/75/100-percent top-layer opacity selected by bits 1:0 |
| `0x7030` | `P_FADE` | Subtracted from each final 8-bit RGB channel with clamping |
...
| `0x703c` | `TV_SATURATION` | Final-output saturation; `0x0020` is neutral |
...
| 8 | `0x0100` | Blend request using the level selected by `P_BLENDING` bits 1:0 |
...
- Pixel bit 15 set is transparent; clear pixels are opaque RGB555.
...
| 2 | Select individual 6-bit blend levels from the sprite extension bank |
...
When `SPRITE_CTRL bit 1` is set, the low ten bits of X and Y are interpreted as
signed direct top-left coordinates (`-512..511`). Pixels outside the LCD are
clipped; sprites do not wrap to the opposite edge.

When clear, the same signed ten-bit fields use a centered coordinate transform:
...
SY's lower-screen objects prove the signed encoding (for example, `0x39a` is
`-102`), while an SY object animated above the top edge proves that the
transformed result is clipped rather than masked back into the screen.

For new homebrew, set bit 1 and use ordinary top-left coordinates in the signed
ten-bit range.
...
When sprite-control bit 2 selects individual blending, extension-word bits
13:8 provide a 6-bit opacity value. The renderer converts that field to the
5-bit alpha used by the GPL compositor. Otherwise, blended sprites use the
global `P_BLENDING` level.
...
Y raw: 0x0d5c at screen y=0, down to 0x02b6 at y=239
```
...
static int touch_raw_to_y(unsigned raw)
{
    ...
    int y = (ymax - (int)raw) * 239 / (ymax - ymin);
...
- The full recovered keyboard matrix is mapped, but undocumented matrix cells
  may still exist.
...
- Direct DAC and the main SPU PCM/ADPCM formats produce SDL audio and advance
  in silent/headless runs.
- Exact ADPCM36 edge cases, envelopes, and analog output characteristics are
...
- USB device enumeration, bulk-only SCSI, and the recovered VTech command
  transport are modeled; arbitrary USB classes and host timing are not.
...
| `702a` | Global blend level |
| `7030` | Final RGB fade |
| `703a` | Palette control |
| `703c` | Final RGB saturation |
| `7042` | Sprite control |
| `7050` | TFT control |
...
This guide is maintained against the emulator sources in this repository. To
record the exact source revision used for a hardware comparison, run:

```sh
shasum -a 256 emulator/src/*.{cpp,hpp}
```
