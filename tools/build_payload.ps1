...
$BundledIde = Join-Path $Root 'compiler\windows\unSPIDE_4.1.1'
$Ide = if ($env:UNSP_IDE) {
    $env:UNSP_IDE
...
$ary = Join-Path $Build 'MobiGo2Starter.ary'
@"
Obj: "$Build\main.obj"
Lib: "$Library"
PrjPath: "$Root\"
...
Invoke-Native (Join-Path $Toolchain 'udocc.exe') @(
    ...
    $include, '-o', (Join-Path $Build 'main.asm'),
    (Join-Path $Root 'src\main.c')
)
Invoke-Native (Join-Path $Toolchain 'xasm16.exe') @(
    '-t4', '-sr', '-wpop', $include,
    '-o', (Join-Path $Build 'main.obj'),
    (Join-Path $Build 'main.asm')
)
...
$map = Join-Path $Build 'MobiGo2Starter.map'
