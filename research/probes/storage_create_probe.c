...
enum {
    ...
    ST_STORAGE_REFRESH = 21,
    ST_EXISTS_AFTER_REFRESH = 22
};

typedef int (*resident_storage_refresh_fn)(void);
#define RESIDENT_STORAGE_REFRESH ((resident_storage_refresh_fn)0x00075fbeUL)

/*
 * Research probe for the missing-file creation path. Retail resident code
 * contains a directory-entry allocation branch, but current emulator NAND
 * FTL behavior does not make the new name visible after close.
 */
static const char test_path[] = "A:DEGER\\HBTEST2.DAT";
...
static int app_start(void)
{
    ...
    if (APP_STATE[ST_EXISTS_BEFORE] != 0) {
    ...
    APP_STATE[ST_EXISTS_AFTER] =
        (mg_sdk_u16)mg_sdk_resident_storage_path_exists(test_path);
    if (APP_STATE[ST_EXISTS_AFTER] != 1) {
        /*
         * Research-only experiment: 0x075fbe selects the active storage
         * backend, runs a backend-specific refresh, commits backend state,
         * and recomputes mounted-volume availability. It is not yet part of
         * the public SDK API. Test whether a newly allocated directory entry
         * only becomes visible after this operation.
         */
        APP_STATE[ST_STORAGE_REFRESH] =
            (mg_sdk_u16)RESIDENT_STORAGE_REFRESH();
        APP_STATE[ST_EXISTS_AFTER_REFRESH] =
            (mg_sdk_u16)mg_sdk_resident_storage_path_exists(test_path);
        if (APP_STATE[ST_EXISTS_AFTER_REFRESH] != 1) {
